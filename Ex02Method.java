...
// 3. 리턴타입: 리턴타입이란, 해당 메소드가 종료될때, 그 메소드를 호출했던 곳으로 보내줄 값의 데이터타입을 리턴 타입이라고 한다.
//            리턴타입이 존재할 때에는 해당 메소드 내부에 반드시 return 이라는 키워드를 사용해서, 그 데이터 타입과 일치하는 데이터 타입을 가진
//            값을 반드시 return 해주어야 한다.
//            또한 리턴의 경우, 실행 되는 코드가 되면 메소드를 그 자리에서 곧장 종료시킨다. 
//            만약 해당 메소드가 아무런 값도 보내주지 않을 때에는 우리가 void라고 적어준다.
//            이때는 오히려 return을 적으면 에러가 발생한다.

// 4. 이름: 소문자로 시작하는 동사

// 5. 파라미터: 파라미터란, 해당 메소드를 실행 시킬때 외부로부터 꼭 받아와야 하는 특정한 종류의 데이터가 있을 경우
...
public class Ex02Method {
    public static void main(String[] args) {
        ...
        // int a, int b 라고 했지만
        // 그것은 메소드 내부에서 해당 파라미터를 어떻개 부를지를 지정해주는 것이라고 보면 좋다.
