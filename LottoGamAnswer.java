...
public class LottoGamAnswer {
    ...
    public static void main(String[] args) {
        ...
        // index 변수에 할당된 값이 SIZE 보다 작은 동안,
        // while을 이용하여 computerArray의 index번 인덱스에 random이 생성한 숫자를
        // 기존 배열에 없을 때에만 추가하고 index 값을 1 증가시킨다.
        while (index < NUMBER_SIZE) {
            ...
            // boolean inputSwitch를 만들고 true로 초기화해준다.
            boolean inputSwitch = true;

            // for문을 사용하여, randomNumber가 computerArray의 엘리먼트 중
        ...
        for (int i = 0; i < userArray.length; i++) {
            ...
            if (userChoice == 1) {
                ...
                while (index < userArray[i].length) {
                    // 사용자한테 숫자를 선택 받아서
                    // 1~45의 범위에 속하고 중복되지 앟은 숫자인지 체크하여
                    // 아닐경우에는 다시 입력해달라고 하자
                ...
                // 사용자가 자동으로 골랐을 경우
                // 컴퓨터와 랜덤숫자 코드와 매우 유사하지만
                // 저장되는 위치가 computerArray[index]가 아니라
                // userArray[i][index] 로 바뀔 뿐이다.
                ...
                while (index < NUMBER_SIZE) {
                    ...
                    // for문을 사용하여, randomNumber가 userArray의 엘리먼트 중
                    // 같은 값을 가진 엘리먼트가 있으면 inputSwitch를 false로 바꾸고
                    // for 문을 break한다.
        ...
        for (int i = 0; i < userArray.length; i++) {
            ...
            System.out.printf("게임 %2d번: [%2d, %2d, %2d, %2d, %2d, %2d ] 맞춘 갯수: %2d개,", i + 1, userArray[i][0],
